...
class EKImagePickerHelper: NSObject {    

    private(set) var albums    = [[String: PHFetchResult<PHAsset>]]()
    private(set) var selectedPhotos = [PHAsset]()
    ...
    class public var pickerPhotoSize: CGSize {
        ...
        let width = (screenWidth - CGFloat(4) * (CGFloat(5))) / CGFloat(4)
    ...
    public func getAblumTitle(_ ablumIndex: Int) -> String {
        if let ablum = self.albums[safe: ablumIndex] {
    ...
    public func isContains(formSelect alasset: PHAsset) -> Bool {
        return selectedPhotos.contains(alasset)
    }
    ...
    public func index(ofSelect alasset: PHAsset) -> Int? {
        return selectedPhotos.index(of: alasset)
    }
    ...
    func remove(formSelect alasset: PHAsset) {
        if let index = index(ofSelect: alasset) {
            selectedPhotos.removeSafe(at: index)
        }
    }
...
//                    let image_buffer = WCLImagePickerBundle.imageFromBundle("image-buffer")
    ...
    public func getPhotoDefalutSize(_ ablumIndex: Int, photoIndex: Int, resultHandler: ((UIImage?, [AnyHashable: Any]?) -> Void)?) {
        getPhoto(ablumIndex, photoIndex: photoIndex, photoSize: EKImagePickerHelper.pickerPhotoSize, resultHandler: resultHandler)
    }
    ...
    private func getPhotoAlbum() {
        ...
        //将获取的相册加入到相册的数组中
        self.albums.append(["全部照片" : allResult])
        ...
        userResult.enumerateObjects(options: .concurrent) { (collection, index, stop) in
            ...
            if assetResult.count != 0 {
                self.albums.append([assetcollection.localizedTitle!:assetResult])
            }
        }
        ...
        smartResult.enumerateObjects(options: .concurrent) { (collection, index, stop) in
            ...
            if assetResult.count != 0 {
                self.albums.append([collection.localizedTitle!:assetResult])
            }
        }
    }
}
